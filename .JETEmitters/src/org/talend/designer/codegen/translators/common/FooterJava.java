...
public class FooterJava
{
  ...
  protected final String TEXT_59 = NL + "\t\t\tif(enableLogStash) {" + NL + "\t\t\t\tjava.util.Properties properties_";
  protected final String TEXT_60 = " = new java.util.Properties();" + NL + "\t\t\t\tproperties_";
  protected final String TEXT_61 = ".setProperty(\"root.logger\", \"audit\");" + NL + "\t\t\t\tproperties_";
  protected final String TEXT_62 = ".setProperty(\"encoding\", \"UTF-8\");" + NL + "\t\t\t\tproperties_";
  protected final String TEXT_63 = ".setProperty(\"application.name\", \"Talend Studio\");" + NL + "\t\t\t\tproperties_";
  protected final String TEXT_64 = ".setProperty(\"service.name\", \"Talend Studio Job\");" + NL + "\t\t\t\tproperties_";
  protected final String TEXT_65 = ".setProperty(\"instance.name\", \"Talend Studio Job Instance\");" + NL + "\t\t\t\tproperties_";
  protected final String TEXT_66 = ".setProperty(\"propagate.appender.exceptions\", \"none\");" + NL + "\t\t\t\tproperties_";
  protected final String TEXT_67 = ".setProperty(\"log.appender\", \"file\");" + NL + "\t\t\t\tproperties_";
  protected final String TEXT_68 = ".setProperty(\"appender.file.path\", ";
  protected final String TEXT_69 = ");" + NL + "\t\t\t\tproperties_";
  protected final String TEXT_70 = ".setProperty(\"appender.file.maxsize\", \"52428800\");" + NL + "\t\t\t\tproperties_";
  protected final String TEXT_71 = ".setProperty(\"appender.file.maxbackup\", \"20\");" + NL + "\t\t\t\tproperties_";
  protected final String TEXT_72 = ".setProperty(\"host\", \"false\");" + NL + "" + NL + "\t\t\t\tSystem.getProperties().stringPropertyNames().stream()" + NL + "\t\t\t\t\t.filter(it -> it.startsWith(\"monitoring.audit.logger.properties.\"))" + NL + "\t\t\t\t\t.forEach(key -> properties_";
  protected final String TEXT_73 = ".setProperty(key.substring(\"monitoring.audit.logger.properties.\".length()), System.getProperty(key)));" + NL + "" + NL + "\t\t\t\t";
  protected final String TEXT_74 = NL + "\t\t\t\torg.apache.log4j.Logger.getLogger(\"audit\").setLevel(org.apache.log4j.Level.DEBUG);" + NL + "\t\t\t\t";
  protected final String TEXT_75 = NL + "\t\t\t\torg.apache.logging.log4j.core.config.Configurator.setLevel(\"audit\", org.apache.logging.log4j.Level.DEBUG);" + NL + "\t\t\t\t";
  protected final String TEXT_76 = NL + "\t\t\t\tauditLogger_";
  protected final String TEXT_77 = " = org.talend.job.audit.JobEventAuditLoggerFactory.createJobAuditLogger(properties_";
  protected final String TEXT_78 = ");" + NL + "\t\t\t}" + NL + "\t\t";
  protected final String TEXT_79 = NL + NL + "        if(clientHost == null) {" + NL + "            clientHost = defaultClientHost;" + NL + "        }" + NL + "" + NL + "        if(pid == null || \"0\".equals(pid)) {" + NL + "            pid = TalendString.getAsciiRandomString(6);" + NL + "        }" + NL + "" + NL + "        if (rootPid==null) {" + NL + "            rootPid = pid;" + NL + "        }" + NL + "        if (fatherPid==null) {" + NL + "            fatherPid = pid;" + NL + "        }else{" + NL + "            isChildJob = true;" + NL + "        }" + NL;
  ...
  protected final String TEXT_81 = NL + NL + "        try {" + NL + "            //call job/subjob with an existing context, like: --context=production. if without this parameter, there will use the default context instead." + NL + "            java.io.InputStream inContext = ";
  protected final String TEXT_82 = ".class.getClassLoader().getResourceAsStream(\"";
  protected final String TEXT_83 = "/contexts/\" + contextStr + \".properties\");" + NL + "            if (inContext == null) {" + NL + "                inContext = ";
  protected final String TEXT_84 = ".class.getClassLoader().getResourceAsStream(\"config/contexts/\" + contextStr + \".properties\");" + NL + "            }" + NL + "            if (inContext != null) {" + NL + "                //defaultProps is in order to keep the original context value" + NL + "                if(context != null && context.isEmpty()) {" + NL + "\t                defaultProps.load(inContext);" + NL + "\t                context = new ContextProperties(defaultProps);" + NL + "                }" + NL + "                " + NL + "                inContext.close();" + NL + "            } else if (!isDefaultContext) {" + NL + "                //print info and job continue to run, for case: context_param is not empty." + NL + "                System.err.println(\"Could not find the context \" + contextStr);" + NL + "            }" + NL + "" + NL + "            if(!context_param.isEmpty()) {" + NL + "                context.putAll(context_param);" + NL + "\t\t\t\t//set types for params from parentJobs" + NL + "\t\t\t\tfor (Object key: context_param.keySet()){" + NL + "\t\t\t\t\tString context_key = key.toString();" + NL + "\t\t\t\t\tString context_type = context_param.getContextType(context_key);" + NL + "\t\t\t\t\tcontext.setContextType(context_key, context_type);" + NL + "" + NL + "\t\t\t\t}" + NL + "            }";
  protected final String TEXT_85 = NL + "            class ContextProcessing {" + NL + "                private void processContext_0() {";
  protected final String TEXT_86 = NL + "                        }" + NL + "" + NL + "                private void processContext_";
  protected final String TEXT_87 = "() {";
  protected final String TEXT_88 = NL + "                        context.setContextType(\"";
  protected final String TEXT_89 = "\", \"";
  protected final String TEXT_90 = NL + "                            String pwd_";
  protected final String TEXT_91 = "_value = context.getProperty(\"";
  protected final String TEXT_92 = "\");" + NL + "                            context.";
  protected final String TEXT_93 = " = null;" + NL + "                            if(pwd_";
  protected final String TEXT_94 = "_value!=null) {" + NL + "                                if(context_param.containsKey(\"";
  protected final String TEXT_95 = "\")) {//no need to decrypt if it come from program argument or parent job runtime" + NL + "                                    context.";
  protected final String TEXT_96 = " = pwd_";
  protected final String TEXT_97 = "_value;" + NL + "                                } else if (!pwd_";
  protected final String TEXT_98 = "_value.isEmpty()) {" + NL + "                                    try {" + NL + "                                        context.";
  protected final String TEXT_99 = " = routines.system.PasswordEncryptUtil.decryptPassword(pwd_";
  protected final String TEXT_100 = "_value);" + NL + "                                        context.put(\"";
  protected final String TEXT_101 = "\",context.";
  protected final String TEXT_102 = ");" + NL + "                                    } catch (java.lang.RuntimeException e) {" + NL + "                                        //do nothing" + NL + "                                    }" + NL + "                                }" + NL + "                            }";
  protected final String TEXT_103 = NL + "                        try{" + NL + "                            String context_";
  protected final String TEXT_104 = "\");" + NL + "                            if (context_";
  protected final String TEXT_105 = "_value == null){" + NL + "                                context_";
  protected final String TEXT_106 = "_value = \"\";" + NL + "                            }" + NL + "                            int context_";
  protected final String TEXT_107 = "_pos = context_";
  protected final String TEXT_108 = "_value.indexOf(\";\");" + NL + "                            String context_";
  protected final String TEXT_109 = "_pattern =  \"yyyy-MM-dd HH:mm:ss\";" + NL + "                            if(context_";
  protected final String TEXT_110 = "_pos > -1){" + NL + "                                context_";
  protected final String TEXT_111 = "_pattern = context_";
  protected final String TEXT_112 = "_value.substring(0, context_";
  protected final String TEXT_113 = "_pos);" + NL + "                                context_";
  protected final String TEXT_114 = "_value = context_";
  protected final String TEXT_115 = "_value.substring(context_";
  protected final String TEXT_116 = "_pos + 1);" + NL + "                            }" + NL + "" + NL + "                            context.";
  protected final String TEXT_117 = "=(java.util.Date)(new java.text.SimpleDateFormat(context_";
  protected final String TEXT_118 = "_pattern).parse(context_";
  protected final String TEXT_119 = "_value));" + NL + "" + NL + "                        } catch(ParseException e) {";
  protected final String TEXT_120 = NL + "                                log.warn(String.format(\"";
  protected final String TEXT_121 = "\", e.getMessage()));";
  protected final String TEXT_122 = NL + "                                System.err.println(String.format(\"";
  protected final String TEXT_123 = NL + "                            context.";
  protected final String TEXT_124 = "=null;" + NL + "                        }";
  protected final String TEXT_125 = "=(";
  protected final String TEXT_126 = ") context.getProperty(\"";
  protected final String TEXT_127 = "= new java.text.StringCharacterIterator(context.getProperty(\"";
  protected final String TEXT_128 = "\")).first();";
  protected final String TEXT_129 = NL + "                            try{" + NL + "                                context.";
  protected final String TEXT_130 = "=routines.system.ParserUtils.parseTo_";
  protected final String TEXT_131 = " (context.getProperty(\"";
  protected final String TEXT_132 = "\"));" + NL + "                            } catch(NumberFormatException e){";
  protected final String TEXT_133 = NL + "                                context.";
  protected final String TEXT_134 = "=null;" + NL + "                            }";
  protected final String TEXT_135 = NL + "                } ";
  protected final String TEXT_136 = NL + "                public void processAllContext() {";
  protected final String TEXT_137 = NL + "                        processContext_";
  protected final String TEXT_138 = "();";
  protected final String TEXT_139 = NL + "                }" + NL + "            }" + NL + "" + NL + "            new ContextProcessing().processAllContext();" + NL + "        } catch (java.io.IOException ie) {" + NL + "            System.err.println(\"Could not load context \"+contextStr);" + NL + "            ie.printStackTrace();" + NL + "        }" + NL + "" + NL + "        // get context value from parent directly" + NL + "        if (parentContextMap != null && !parentContextMap.isEmpty()) {";
  protected final String TEXT_140 = "if (parentContextMap.containsKey(\"";
  protected final String TEXT_141 = "\")) {" + NL + "                context.";
  protected final String TEXT_142 = " = (";
  protected final String TEXT_143 = ") parentContextMap.get(\"";
  ...
  protected final String TEXT_146 = NL + "\t\t\tparametersToEncrypt.add(\"";
  protected final String TEXT_147 = NL + "        //Resume: jobStart" + NL + "        resumeUtil.addLog(\"JOB_STARTED\", \"JOB:\" + jobName, parent_part_launcher, Thread.currentThread().getId() + \"\", \"\",\"\",\"\",\"\",resumeUtil.convertToJsonText(context,parametersToEncrypt));" + NL;
  ...
  protected final String TEXT_150 = NL + "    try {" + NL + "        runTrace.openSocket(!isChildJob);" + NL + "        runTrace.startThreadTrace(clientHost, portTraces);" + NL + "        if (runTrace.isPause()) {" + NL + "\t\t\twhile (runTrace.isPause()) {" + NL + "\t\t\t\tThread.sleep(100);" + NL + "\t\t\t}" + NL + "\t\t}" + NL + "    } catch (java.io.IOException ioException) {" + NL + "        ioException.printStackTrace();" + NL + "    } catch (InterruptedException e) {" + NL + "\t\t\te.printStackTrace();" + NL + "\t}";
  protected final String TEXT_151 = NL + "\t";
  protected final String TEXT_152 = NL + "\t    java.util.concurrent.ConcurrentHashMap<Object, Object> concurrentHashMap = new java.util.concurrent.ConcurrentHashMap<Object, Object>();" + NL + "\t    globalMap.put(\"concurrentHashMap\", concurrentHashMap);" + NL + "\t";
  protected final String TEXT_153 = NL + NL + "    long startUsedMemory = Runtime.getRuntime().totalMemory() - Runtime.getRuntime().freeMemory();" + NL + "    long endUsedMemory = 0;" + NL + "    long end = 0;" + NL + "" + NL + "    startTime = System.currentTimeMillis();";
  protected final String TEXT_154 = NL + "        java.net.ProxySelector proxySelectorOnStartJob = java.net.ProxySelector.getDefault();";
  protected final String TEXT_155 = ".addMessage(\"begin\");";
  protected final String TEXT_156 = NL + "    System.err.println(\"Only one hadoop configuration is allowed in one job!\");";
  protected final String TEXT_157 = NL + NL + "this.globalResumeTicket = true;//to run tPreJob" + NL;
  ...
  protected final String TEXT_162 = NL + "\t\tif(enableLogStash) {" + NL + "\t        ";
  protected final String TEXT_163 = ".addJobStartMessage();" + NL + "\t        try {" + NL + "\t            ";
  protected final String TEXT_164 = "Process(globalMap);" + NL + "\t        } catch (java.lang.Exception e) {" + NL + "\t            e.printStackTrace();" + NL + "\t        }" + NL + "        }";
  protected final String TEXT_165 = NL + NL + "this.globalResumeTicket = false;//to run others jobs" + NL;
  protected final String TEXT_166 = "final Thread launchingThread = Thread.currentThread();";
  protected final String TEXT_167 = NL + "        runningThreadCount.add(1);" + NL + "        new Thread(){" + NL + "            public void run() {" + NL + "                java.util.Map threadRunResultMap = new java.util.HashMap();" + NL + "                threadRunResultMap.put(\"errorCode\", null);" + NL + "                threadRunResultMap.put(\"status\", \"\");" + NL + "                threadLocal.set(threadRunResultMap);" + NL;
  protected final String TEXT_168 = NL + "                ";
  protected final String TEXT_169 = NL + "                finally {" + NL + "                    Integer localErrorCode = (Integer)(((java.util.Map)threadLocal.get()).get(\"errorCode\"));" + NL + "                    String localStatus = (String)(((java.util.Map)threadLocal.get()).get(\"status\"));" + NL + "                    if (localErrorCode != null) {" + NL + "                        if (errorCode == null || localErrorCode.compareTo(errorCode) > 0) {" + NL + "                           errorCode = localErrorCode;" + NL + "                        }" + NL + "                    }" + NL + "                    if (!status.equals(\"failure\")){" + NL + "                        status = localStatus;" + NL + "                    }" + NL + "" + NL + "                    if (\"true\".equals(((java.util.Map) threadLocal.get()).get(\"JobInterrupted\"))) {" + NL + "                        launchingThread.interrupt();" + NL + "                    }" + NL + "" + NL + "                    runningThreadCount.add(-1);" + NL + "                }" + NL + "            }" + NL + "        }.start();" + NL;
  ...
  protected final String TEXT_171 = NL + NL + "this.globalResumeTicket = true;//to run tPostJob" + NL;
  protected final String TEXT_172 = NL + "        java.net.ProxySelector.setDefault(proxySelectorOnStartJob);";
  ...
  protected final String TEXT_175 = ".addMessage(status==\"\"?\"end\":status, (end-startTime));" + NL + "        try {";
  protected final String TEXT_176 = ".addJobEndMessage(startTime, end, status);" + NL + "\t        try {" + NL + "\t            ";
  protected final String TEXT_177 = NL + "    closeJmsConnections();";
  protected final String TEXT_178 = NL + "    closeScpConnections();";
  protected final String TEXT_179 = NL + "if (execStat) {" + NL + "    runStat.updateStatOnJob(RunStat.JOBEND, fatherNode);" + NL + "    runStat.stopThreadStat();" + NL + "}";
  protected final String TEXT_180 = NL + "        runTrace.stopThreadTrace();";
  protected final String TEXT_181 = NL + "    int returnCode = 0;" + NL + "    if(errorCode == null) {" + NL + "         returnCode = status != null && status.equals(\"failure\") ? 1 : 0;" + NL + "    } else {" + NL + "         returnCode = errorCode.intValue();" + NL + "    }" + NL + "    resumeUtil.addLog(\"JOB_ENDED\", \"JOB:\" + jobName, parent_part_launcher, Thread.currentThread().getId() + \"\", \"\",\"\" + returnCode,\"\",\"\",\"\");" + NL + "" + NL + "    return returnCode;" + NL + "" + NL + "  }" + NL + "" + NL + "    // only for OSGi env" + NL + "    public void destroy() {";
  protected final String TEXT_182 = NL + "    closeSqlDbConnections();";
  protected final String TEXT_183 = NL + "    closeCassandraConnections();";
  protected final String TEXT_184 = NL + "    closeHBaseConnections();";
  protected final String TEXT_185 = NL + "    closeFtpConnections();";
  protected final String TEXT_186 = NL + "    closeS3Connections();";
  protected final String TEXT_187 = NL + "    closeLDAPConnections();";
  protected final String TEXT_188 = NL + "    closeMDMConnections();";
  protected final String TEXT_189 = NL + "    closePaloConnections();";
  protected final String TEXT_190 = NL + "    closeEXistConnections();";
  protected final String TEXT_191 = NL + "    closeGSConnections();";
  protected final String TEXT_192 = NL + NL + NL + "    }" + NL;
  protected final String TEXT_193 = NL + "    private void closeJmsConnections() {" + NL + "        try {" + NL + "            Object obj_conn;";
  protected final String TEXT_194 = NL + "            obj_conn = globalMap.remove(\"connection_";
  protected final String TEXT_195 = "\");" + NL + "            if (null != obj_conn) {" + NL + "                ((javax.jms.Connection) obj_conn).close();" + NL + "            }";
  protected final String TEXT_196 = NL + "        } catch (java.lang.Exception e) {" + NL + "        }" + NL + "    }";
  protected final String TEXT_197 = NL + "    private void closeScpConnections() {" + NL + "        try {" + NL + "            Object obj_conn;";
  protected final String TEXT_198 = NL + "            obj_conn = globalMap.remove(\"conn_";
  protected final String TEXT_199 = "\");" + NL + "            if (null != obj_conn) {" + NL + "                ((ch.ethz.ssh2.Connection) obj_conn).close();" + NL + "            }";
  protected final String TEXT_200 = NL + "    private void closeSqlDbConnections() {" + NL + "        try {" + NL + "            Object obj_conn;";
  protected final String TEXT_201 = "\");" + NL + "            if (null != obj_conn) {" + NL + "                ((java.sql.Connection) obj_conn).close();" + NL + "            }";
  protected final String TEXT_202 = NL + "    private void closeCassandraConnections() {" + NL + "        try {" + NL + "            Object obj_cluster;" + NL + "            Object obj_conn;";
  protected final String TEXT_203 = NL + "            obj_cluster = globalMap.remove(\"cluster_";
  protected final String TEXT_204 = "\");" + NL + "            obj_conn = globalMap.remove(\"connection_";
  protected final String TEXT_205 = "\");" + NL + "            if(obj_conn != null) {" + NL + "                ((com.datastax.driver.core.Session) obj_conn).close();" + NL + "            }" + NL + "            if(obj_cluster != null) {" + NL + "                ((com.datastax.driver.core.Cluster) obj_cluster).close();" + NL + "            }";
  protected final String TEXT_206 = NL + "            obj_conn = globalMap.remove(\"cluster_";
  protected final String TEXT_207 = "\");" + NL + "            if(obj_conn!=null) {" + NL + "                ((me.prettyprint.hector.api.Cluster) obj_conn).getConnectionManager().shutdown();" + NL + "            }";
  protected final String TEXT_208 = NL + "    private void closeHBaseConnections() {" + NL + "        try {";
  protected final String TEXT_209 = NL + "   \t\t\torg.apache.hadoop.conf.Configuration conn_";
  protected final String TEXT_210 = " =(org.apache.hadoop.conf.Configuration)globalMap.get(\"conn_";
  protected final String TEXT_211 = "\");" + NL + "\t\t\tif(conn_";
  protected final String TEXT_212 = " != null )" + NL + "\t        {" + NL + "\t\t\t";
  protected final String TEXT_213 = NL + "                                org.apache.hadoop.hbase.client.HConnection hConnection_";
  protected final String TEXT_214 = " = org.apache.hadoop.hbase.client.HConnectionManager.getConnection(conn_";
  protected final String TEXT_215 = NL + "                                org.apache.hadoop.hbase.client.Connection hConnection_";
  protected final String TEXT_216 = " = org.apache.hadoop.hbase.client.ConnectionFactory.createConnection(conn_";
  protected final String TEXT_217 = NL + "\t\t        if ((hConnection_";
  protected final String TEXT_218 = " != null) && (!hConnection_";
  protected final String TEXT_219 = ".isClosed())) {" + NL + "\t\t\t\t    hConnection_";
  protected final String TEXT_220 = ".close();" + NL + "\t\t\t}" + NL + "\t\t\t";
  protected final String TEXT_221 = NL + "\t\t\t\torg.apache.hadoop.hbase.client.HConnectionManager.deleteConnection(conn_";
  protected final String TEXT_222 = ",true);" + NL + "\t\t\t";
  protected final String TEXT_223 = NL + "\t\t\t}" + NL + "      globalMap.remove(\"conn_";
  protected final String TEXT_224 = "\");" + NL + "\t";
  protected final String TEXT_225 = NL + "    private void closeFtpConnections() {" + NL + "        try {" + NL + "            Object obj_conn;";
  protected final String TEXT_226 = "\");" + NL + "            if (obj_conn != null) {";
  protected final String TEXT_227 = NL + "                ((com.enterprisedt.net.ftp.FTPClient) obj_conn).quit();";
  protected final String TEXT_228 = NL + "                ((com.jcraft.jsch.ChannelSftp) obj_conn).quit();";
  protected final String TEXT_229 = NL + "                ((org.apache.commons.net.ftp.FTPSClient) obj_conn).disconnect();";
  protected final String TEXT_230 = NL + "            }";
  protected final String TEXT_231 = NL + "    private void closeS3Connections() {" + NL + "        try {";
  protected final String TEXT_232 = NL + "    \tcom.amazonaws.services.s3.AmazonS3Client conn_";
  protected final String TEXT_233 = " = (com.amazonaws.services.s3.AmazonS3Client)globalMap.get(\"conn_";
  protected final String TEXT_234 = "\");" + NL + "\t    if(conn_";
  protected final String TEXT_235 = " !=null){" + NL + "\t\t\tconn_";
  protected final String TEXT_236 = ".shutdown();" + NL + "\t\t}";
  protected final String TEXT_237 = NL + "    private void closeLDAPConnections() {" + NL + "        try {" + NL + "            Object obj_conn;";
  protected final String TEXT_238 = "\");" + NL + "            if (obj_conn != null) {" + NL + "                ((javax.naming.ldap.InitialLdapContext) obj_conn).close();" + NL + "            }";
  protected final String TEXT_239 = NL + "    private void closeMDMConnections() {" + NL + "        try {" + NL + "            java.util.Iterator keySets = globalMap.keySet().iterator();" + NL + "            while (keySets.hasNext()) {" + NL + "                String key = keySets.next().toString();" + NL + "                if (key.startsWith(\"TMDMService_tMDMConnection_\")) {" + NL + "                    org.talend.mdm.webservice.TMDMService service = (org.talend.mdm.webservice.TMDMService)globalMap.get(key);" + NL + "                    if (service != null) {" + NL + "                        service.logout(new org.talend.mdm.webservice.WSLogout());" + NL + "                    }" + NL + "                }" + NL + "            }" + NL + "        } catch (java.lang.Exception e) {" + NL + "        }" + NL + "    }";
  protected final String TEXT_240 = NL + "    private void closePaloConnections() {" + NL + "        try {";
  protected final String TEXT_241 = NL + "    \torg.talend.jpalo.paloconnection pConn_";
  protected final String TEXT_242 = " = (org.talend.jpalo.paloconnection)globalMap.get(\"pConn_";
  protected final String TEXT_243 = "\");" + NL + "\t    if(pConn_";
  protected final String TEXT_244 = " !=null){" + NL + "\t\t\t\t\tpConn_";
  protected final String TEXT_245 = ".logout();" + NL + "" + NL + "\t\t}";
